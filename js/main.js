...
document.addEventListener('DOMContentLoaded', () => {
    const COUNTDOWN_KEY = 'countdownDeadline'; // Clé pour stocker la date limite
    const COUNTDOWN_DURATION = 16 * 24 * 60 * 60 * 1000; // 16 jours en millisecondes

    let countdownDate;

    // Vérifier si la date limite existe déjà dans localStorage
    const savedCountdown = localStorage.getItem(COUNTDOWN_KEY);
    if (savedCountdown) {
        countdownDate = new Date(savedCountdown);
    } else {
        // Si aucune date limite, la définir à 16 jours à partir de maintenant
